...
| 表 | 关键字段 | 作用 |
|---|---|---|
| `events` | `id`, `onchain_event_id`, `creator`, `question`, `description`, `category_id`, `status`, `created_at` | 事件基础表 |
| `markets` | `id`, `event_id`, `onchain_event_id`, `market_idx`, `title`, `label_yes/no`, `deadline`, `yes_pool/no_pool/bonus_pool`, `proposed_outcome`, `proposed_by`, `finalized_outcome`, `dispute_active`, `creator_seed_bet`, `min_bet`, 费率快照字段, `token_mint`, `vault`, `status` | 子市场 + 状态 + pool + 配置快照 |
| `positions` | `market_id`, `user_address`, `yes_amount`, `no_amount`, `claimed`, `payout` | 用户聚合持仓 |
| `trades` | `market_id`, `user_address`, `side`, `amount`, `tx_signature`, `slot`, `block_time` | 每次下注流水 |
| `disputes` | `market_id`, `disputer`, `claimed_outcome`, `bond_amount`, `bond_tx_signature`, `reason`, `resolved_outcome`, `status` | 争议记录 |
...
| `configs` | `id`, `value`, `updated_at` | 全局 KV 配置（平台费、奖励比例、窗口时长等） |
...
| `event_categories` | 分类定义（MVP 阶段 category 可先写死或存字符串） |
| `tags` / `event_tags` | 标签 |
| `activities` | banner/运营位 |
| `media` | 图片资源 |
| `notifications` | 通知中心 |
| `referrals` / `referral_rewards` | 邀请关系与奖励 |
