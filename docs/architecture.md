...
PolyMind 是一个基于 Solana 的预测市场平台。整体采用**链上程序 + Python API Worker + Python API + Web 前端**的分层架构：
...
| 索引器 | Python Worker (`app/workers/`) | Python 轮询 Solana RPC、解析事件并写 PostgreSQL，与旧 PolyMind 架构一致 |
...
│       │       ├── champion_indexer.py     # Champion/H5 活动事件索引
...
| 部署 | `dev.sh` 一键启动 | 旧：含 indexer、notify_cron、gateway；新：web + admin + api + Python workers |
...
| Python 轮询 indexer | Python 轮询 indexer | 监听方式保持一致，仍由 `app/workers/indexer.py` 承担 |
...
### 9.2 Python Indexer 实现

- **位置**：`apps/api/app/workers/indexer.py`。
- **数据源**：Solana RPC `getSignaturesForAddress` + `getTransaction` 轮询。
- **事件解析**：解析 program logs，提取 Anchor event。
- **事件去重**：按 `tx_signature` + `slot` + `event_index`。
- **失败重试**：解析失败会阻塞 cursor 前进，下次轮询重试。
- **backfill 机制**：从 `indexer_cursor` 表记录的 slot/signature 继续同步，可手动重置游标做历史回填。

配套 worker：
- `champion_indexer.py`：监听 Champion 程序事件。
- `notification_worker.py`：消费 `chain_event_log`，生成 `notifications`。
- `deadline_cron.py`：时间触发提醒（deadline 到达、propose 窗口过期等）。
- `referral_reward_worker.py`：市场结算后结算推荐奖励。
...
2. 完善 `app/workers/indexer.py` 的错误处理和可观测性。
